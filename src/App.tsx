...
import FeaturesManager from '@/components/settings/FeaturesManager'
...
function App() {
  ...
  // On first run (no features installed) or pending install queue, show setup overlay
  ...
  useEffect(() => {
    ...
    if (!anyInstalled || hasPendingQueue) {
      console.log('[App] Showing setup overlay:', !anyInstalled ? 'first run' : 'pending install queue')
      setShowSetup(true)
    }
  }, [featuresStatus])
  ...
  return (
    <>
    {/* Full-screen setup overlay — blocks everything until features are installed */}
    {showSetup && (
      <div className="fixed inset-0 z-[9999] bg-background flex items-center justify-center">
        <div className="w-full max-w-md">
          <FeaturesManager onComplete={() => {
            console.log('[App] Setup complete, dismissing overlay')
            setShowSetup(false)
          }} />
        </div>
      </div>

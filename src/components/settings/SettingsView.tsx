...
function ModelsSettings() {
  ...
  // Request GPU info and current status on mount
  useEffect(() => {
    console.log('[ModelsSettings] Mount — requesting gpu_info and get_status')
  ...
  useEffect(() => {
    if (lastMessage?.type === 'gpu_info') {
      console.log('[ModelsSettings] Received gpu_info:', lastMessage.payload)
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {gpu && !gpu.available && gpu.name !== undefined && (() => {
        // Determine torch install type from features store
        const features = featuresStatus?.features
        const torchCpuInstalled = features?.['torch_cpu']?.installed
        const torchCudaInstalled = features?.['torch_cuda']?.installed
        const torchVersion = features?.['torch_cpu']?.version || features?.['torch_cuda']?.version || ''
        const isCpuTorch = torchCpuInstalled && !torchCudaInstalled
        const hasCpuSuffix = torchVersion.includes('+cpu') || torchVersion.includes('cpu')
        console.log('[ModelsSettings] GPU unavailable — torch info:', { torchCpuInstalled, torchCudaInstalled, torchVersion, isCpuTorch, hasCpuSuffix })
        return (
          <div className="rounded-lg bg-yellow-500/10 border border-yellow-500/20 p-3">
            <p className="text-xs text-yellow-400">
              {(isCpuTorch || hasCpuSuffix)
                ? 'CPU-only PyTorch installed. Install CUDA PyTorch in Features for GPU support.'
                : 'GPU not available. Models will run on CPU (slower but works everywhere).'}
            </p>
          </div>
        )
      })()}
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="space-y-3">
        ...
        >
          ...
          {/* OCR Toggle Button (cyan) — always visible */}
          {(() => {
...
function VoiceConversionSettings() {
  ...
  const { updateRVCMicParams, updateRVCTtsParams } = useSettingsStore()
